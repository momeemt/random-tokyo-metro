...
const lines: Record<string, string[]> = {
  ...
  // 都営大江戸線（光が丘→都庁前→環状部→都庁前）
  E_radial: ['光が丘', '練馬春日町', '豊島園', '練馬', '新江古田', '落合南長崎', '中井', '東中野', '中野坂上', '西新宿五丁目', '都庁前'],
  E_loop: ['都庁前', '新宿西口', '東新宿', '若松河田', '牛込柳町', '牛込神楽坂', '飯田橋', '春日', '本郷三丁目', '上野御徒町', '新御徒町', '蔵前', '両国', '森下', '清澄白河', '門前仲町', '月島', '勝どき', '築地市場', '汐留', '大門', '赤羽橋', '麻布十番', '六本木', '青山一丁目', '国立競技場', '代々木', '新宿', '都庁前'],
}
...
const transfers: string[][] = [
  ...
  ['大手町'],    // M, T, C, Z, I
  ['神保町'],    // Z, I, S
  ['春日', '後楽園'], // I, E と M, N（春日と後楽園は乗り換え可能）
  ['新宿'],      // M, S, E
  ['新宿三丁目'], // M, F, S
  ['市ケ谷'],    // Y, N, S
  ['九段下'],    // T, Z, S
  ...
  ['麻布十番'], // N, E
  ['東新宿'],   // F, E
  ['上野御徒町', '上野広小路', '仲御徒町'], // E, G, H（徒歩乗り換え）
  ['岩本町', '秋葉原'], // S, H（徒歩乗り換え）
  ['馬喰横山', '東日本橋'], // S, A（徒歩乗り換え）
...
const lineColors: Record<string, { name: string; color: string }> = {
  ...
  A: { name: '都営浅草線', color: '#E85298' },
  I: { name: '都営三田線', color: '#0079C2' },
  S: { name: '都営新宿線', color: '#6CBB5A' },
  E_radial: { name: '都営大江戸線', color: '#B6007A' },
